    ...
    public static class BitmapMetadataHelper
    {
        public static T GetQuery<T>(this BitmapMetadata bitmapMetadata, string query)
        {
            ...
            else if (typeof(T) == typeof(TimeSpan))
            {
                ...
                {
                    //TODO: BugBug: PORTISSUE:  included a sample implementation as opposed to throwing 
                    //  NotImplementedException.  Review and revise for accuracy.
                    TimeSpan timeSpan = GetTimeSpan(timespanString);
        ...
        private static TimeSpan GetTimeSpan(string timeSpanString)
        {
            char[] chars = new char[] { '+', '-' };


            DateTimeOffset dto = DateTimeOffset.UtcNow;
            string ts = timeSpanString;
            int index = timeSpanString.IndexOfAny(chars);
            if (index > 0)
            {
                ts = timeSpanString.Substring(0, index);
                string tso = timeSpanString.Substring(index, timeSpanString.Length - index);
                var dtop = DateTimeOffset.TryParseExact(tso, "zzz",
                                 CultureInfo.InvariantCulture,
                                 DateTimeStyles.None, out dto);
            }

            int hour = Convert.ToInt32(ts.Substring(0, 2));
            int minute = Convert.ToInt32(ts.Substring(2, 2));
            int second = Convert.ToInt32(ts.Substring(4, 2));

            DateTime dt = DateTime.Today;
            DateTime dt2 = new DateTime(dt.Year, dt.Month, dt.Day, hour, minute, second);
            DateTime dt3 = dt2.AddHours(dto.Offset.TotalHours);
            TimeSpan timeSpan1 = dt3.TimeOfDay;
